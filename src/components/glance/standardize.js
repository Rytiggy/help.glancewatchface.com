...
export default class standardize {
	...
	bloodsugars(data, extraData, settings) {
		console.log('companion - standardize - bloodsugars()')
		...
		let upbat = '';
		let sage = ''
		console.log(`BGS: ${bgs} !data.error: ${!data.error} data: ${data} bgs !== 'undefined': ${bgs !== 'undefined'}`);
		if (bgs && !data.error && data && bgs !== 'undefined') {
			if (settings.dataSource === 'nightscout') {
				...
				console.log("Standardized dexcom data " + bgsTemplate.bgs)
				bgs = bgsTemplate.bgs;
			...
			console.log('Line 151:  companion - standardize cleanedBgs' + JSON.stringify(cleanedBgs))
			...
			// console.log(JSON.stringify(cleanedBgs))
		...
		console.log('Line 63: here reurning error')
	...
	settings(settings) {
		console.log('Line 212: companion - standardize - settings()');
		// Convert any values to desired units type 
		if (settings.glucoseUnits === 'mmol') {
...
function checkTimeBetweenGraphPoints(bgs, firstNonPredictiveBg) {
	console.log('Line 478: companion - standardize - checkTimeBetweenGraphPoints');
	let firstRun = true;
...
// 	console.log('Dexcom trend: ' + trend)
// 	return trend
	// switch(trend) {
	// 	case 1:
	// 		return "DoubleUp";
